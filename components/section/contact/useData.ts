...
const useData = () => {
  ...
  const validateCallbackNumber = (callBackContact: String) => {
    let requiredField = "";
    if (!callBackContact) {
      requiredField = "Phone number";
    }
    if (callBackContact.length < 10) {
      return `Phone number is invalid.`;
    }
    if (!requiredField) {
      return "";
  ...
  const handleCallbackRequest = async (e: any, callBackContact: String) => {
    e.preventDefault();
    try {
      const formValidationError = validateCallbackNumber(callBackContact);
