...
import { setTransform, setStyles, updateElements, createBlock, reCalculateBlocks } from "./utilities";
...
export default (props: {
  ...
  const animate = () => {
    ...
    if (container.current) setTransform(container.current, "translateY(" + -yCurrent + "px)");
  ...
  const init = () => {
    yScroll = window.scrollY || window.pageYOffset;
    yCurrent = yScroll;
    if (spacer.current && container.current) {
